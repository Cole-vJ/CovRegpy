...
if __name__ == "__main__":
    ...
    time_series_mat = np.vstack((np.sin(5 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                                 np.sin(4 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                                 np.sin(2 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                                 np.sin(8 * (time + 2 * np.pi * np.random.uniform(0, 1)))))

    hilbert_1 = Hilbert(time=time, time_series=time_series_mat[0, :])
    ht_1 = hilbert_1.dtht_fft()
    hilbert_2 = Hilbert(time=time, time_series=time_series_mat[1, :])
    ht_2 = hilbert_2.dtht_fft()
    hilbert_3 = Hilbert(time=time, time_series=time_series_mat[2, :])
    ht_3 = hilbert_3.dtht_fft()
    hilbert_4 = Hilbert(time=time, time_series=time_series_mat[3, :])
    ht_4 = hilbert_4.dtht_fft()
    hts = np.vstack((ht_1, ht_2, ht_3, ht_4))

    hts = np.vstack((np.sin(5 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                     np.sin(4 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                     np.sin(2 * (time + 2 * np.pi * np.random.uniform(0, 1))),
                     np.sin(8 * (time + 2 * np.pi * np.random.uniform(0, 1)))))

    if_1 = omega(time, theta(time_series_mat[0, :], ht_1))
    if_2 = omega(time, theta(time_series_mat[1, :], ht_2))
    if_3 = omega(time, theta(time_series_mat[2, :], ht_3))
    if_4 = omega(time, theta(time_series_mat[3, :], ht_4))
    ifs = np.vstack((if_1, if_2, if_3, if_4))

    temp, _ = CovRegpy_IFF(time=time, imfs=time_series_mat, hts=hts, ifs=ifs, fit=True)

    plt.plot(time, time_series_mat.T)
    plt.plot(_, temp.T)
    plt.show()

    low_frequency = 5 * np.sin(1 * (time + 2 * np.pi * np.random.uniform(0, 1)))
    high_frequency = np.sin(5 * (time + 2 * np.pi * np.random.uniform(0, 1)))
    time_series = low_frequency + high_frequency

    window = 200

    temp = CovRegpy_IMF_IFF(time, time_series, type='linear', components=2, fit_window=1000, forecast_window=window,
                            force_fit=True, debug=False)

    plt.plot(time, low_frequency)
    plt.plot(time, high_frequency)
    plt.show()
    ...
    plt.plot(time, high_frequency)
    plt.plot(time, emd[0][1, :])
    plt.show()

    plt.plot(time, low_frequency)
    plt.plot(time, emd[0][2, :])
    plt.show()

    high_freq_amplitude = np.sqrt(emd[0][1, :] ** 2 + emd[1][1, :] ** 2)
    high_freq_if = emd[2][1, :]
    plt.plot(high_freq_amplitude)
    plt.plot(high_freq_if)
    plt.show()

    low_freq_amplitude = np.sqrt(emd[0][2, :] ** 2 + emd[1][2, :] ** 2)
    low_freq_if = emd[2][2, :]
