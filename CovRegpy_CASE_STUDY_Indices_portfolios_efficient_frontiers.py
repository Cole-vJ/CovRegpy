...
for day in range(len(end_of_month_vector_cumsum[:-int(months + 1)])):
    ...
    # plt.xlabel("Variance")
    # plt.legend(loc='best')
    # plt.title('Efficient Frontiers Calculated using Restricted Bandwith Components')
    # plt.show()
    ...
    for signal in range(np.shape(price_signal)[1]):
        ...
        # plt.plot(ifs[1:int(np.shape(imfs)[0] + 1)].T)
        # plt.show()
    ...
    # plt.scatter(np.sqrt(model_variance_forecast_direct_ssa), v_forecast_direct_ssa,
    ...
    model_variance_forecast_direct_high = global_obj_fun(weights_Model_forecast_direct_high, monthly_covariance)
    model_returns_forecast_direct_high = sum(weights_Model_forecast_direct_high * monthly_returns)
    # plt.scatter(np.sqrt(model_variance_forecast_direct), model_returns_forecast_direct, label='CovReg Direct Model')
    ...
    covariance_storage.append(variance_median_direct)
    covariance_storage.append(variance_median_direct_high)
    covariance_storage.append(variance_median_direct_ssa)

    returns_storage = list()
    returns_storage.append(monthly_returns)
    returns_storage.append(monthly_returns)
    returns_storage.append(monthly_returns)
    # returns_storage.append(model_returns_forecast_direct)
    # returns_storage.append(model_returns_forecast_direct_high)
    # returns_storage.append(model_returns_forecast_direct_ssa)

    colours_storage = list()
    colours_storage.append('cyan')
    colours_storage.append('magenta')
    colours_storage.append('orange')
    colours_storage.append('black')

    name_storage = list()
    name_storage.append(textwrap.fill('Efficient frontier all frequencies', 20))
    name_storage.append(textwrap.fill('Efficient frontier high frequencies', 20))
    name_storage.append(textwrap.fill('Efficient frontier low frequencies', 20))
    name_storage.append(textwrap.fill('Efficient frontier realised', 20))

    for j in range(4):
        if j < 3:
            gm_w, gm_sd, gm_r = global_minimum_forward_applied_information(annual_covariance,
                                                                           covariance_storage[j],
                                                                           returns_storage[j])
            ms_w, ms_sd, ms_r = sharpe_forward_applied_information(annual_covariance,
                                                                   annual_returns,
                                                                   covariance_storage[j],
                                                                   returns_storage[j],
                                                                   risk_free, gm_w, gm_r)
            ef_sd, ef_r = efficient_frontier(gm_w, gm_r, gm_sd, ms_w, ms_r, ms_sd,
                                             covariance_storage[j])
        else:
            gm_w, gm_sd, gm_r = global_minimum_forward_applied_information(annual_covariance,
                                                                           monthly_covariance,
            ...
            ef_sd, ef_r = efficient_frontier(gm_w, gm_r, gm_sd, ms_w, ms_r, ms_sd,
                                             monthly_covariance)
        plt.plot(ef_sd, ef_r, '--', c=colours_storage[j], label=name_storage[j], zorder=1)
    ...
    plt.savefig('figures/efficient_frontiers/Efficient_frontiers_{}.png'.format(int(day + 1)))
    plt.show()
